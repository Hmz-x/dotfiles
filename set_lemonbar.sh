...
call_later()
{
	...
	# don't write alignment_direction
	#echo "$temp_loop_file"
	#echo "function, dir: $passed_func, $passed_dir"
	line_count=$(wc -l "$temp_loop_file" | cut -d ' ' -f 1)

	if ((line_count>0)) && \
	awk "NR==$line_count" "$temp_loop_file" | grep "$passed_dir" -q; then
		#printf -- " %s" "$passed_func" 
		printf -- " %s" "$passed_func" >> "$temp_loop_file"
	else
		# Write alignment_direction and executable_function
